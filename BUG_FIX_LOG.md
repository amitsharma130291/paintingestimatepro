...
## 27. Free estimate template claimed "saved locally in your browser" even when the save silently failed (item 9)

**Found by:** Task item 9, naming the defect directly: "EstimateTemplate.tsx silently catches localStorage failures while displaying 'saved locally in your browser.'" Reading the component confirmed `persistDrafts()` wrapped `localStorage.setItem` in a `try/catch` that discarded any error with only a code comment ("storage unavailable... draft still works for this session"), while the render always showed "· saved locally in your browser." unconditionally — a private-browsing mode, a full quota, or any other storage failure would leave the user believing their estimate was safely saved when it existed only in memory for that tab. Separately, `loadStoredDrafts()` collapsed "never saved anything" and "saved something that's now unreadable" into the same silent `null` result, so a corrupted stored value was discarded and replaced with a fresh blank draft with no indication anything had gone wrong or been lost.

**Fix:** extracted the storage functions into a new `src/components/tools/estimateTemplateStorage.ts` module (independently testable — `EstimateTemplate.tsx`'s JSX means it can't be imported directly by this project's Vitest/esbuild setup, matching the established `*Logic.ts` pure-module pattern already used for calculation rules). `persistDrafts()` now returns `{status: 'saved'} | {status: 'failed', reason}` instead of swallowing the error; `loadStoredDrafts()` returns a three-way `{status: 'loaded'|'empty'|'corrupted'}`, and on `'corrupted'` preserves the original unreadable bytes under a timestamped backup key before falling back to a fresh draft, rather than silently discarding them. The component now tracks real save status in state, shows "not saved yet" (never a false "saved") plus an explicit warning and a **Retry saving** button whenever a write fails, and shows a distinct notice when recovering from corrupted data — explicitly not claiming a normal fresh start. A failed `localStorage.setItem` never partially writes (the platform's own per-key guarantee), so the previously-committed save is always left intact on failure — verified by test, not just asserted.

**Regression tests:** `tests/ui/estimateTemplateStorage.test.ts` (9 tests) against a stubbed `localStorage`: the exact quota-exceeded reproduction (previously silent, now reports `failed`); a corrupted stored value is distinguished from a genuine first visit and its raw bytes are recoverable under the backup key; a well-formed-JSON-wrong-shape value and an empty `drafts` array are both treated as corrupted, never silently accepted; a completely inaccessible storage API (`getItem` itself throwing) is treated as empty rather than crashing; and a failed write is confirmed to leave the prior committed value completely untouched.

**Verification — live in the browser:** typed into the estimate template, then monkey-patched `Storage.prototype.setItem` to throw `QuotaExceededError` for this tool's own storage key and edited a field — the UI immediately showed "Not saved — your browser blocked local storage..." with a **Retry saving** button and "· not saved yet." (previously would have silently shown "· saved locally in your browser."). Restored the real `setItem` and clicked **Retry saving** — the message cleared and the estimate correctly showed "· saved locally in your browser." again, with no page reload needed.

**Verification (automated):** 332/332 tests pass (9 new); `astro check` 0 errors; `astro build` succeeds.

---

## 28. Built a real browser-style component test harness for ProApp (item 11) — no production unlock route

**Found by:** Task item 11, explicitly demanding "Test the real Confirm, Cancel, Keep local, Use imported, and Keep both actions. Merely constructing the desired final records inside a test is insufficient" and "Test this using production transactions and a browser workflow, not only manually constructed record arrays." Every prior integration test in this project (draft/issued isolation, backup restore, import atomicity, version conflicts) drove real storage transactions correctly, but only ever by calling the underlying domain/storage FUNCTIONS directly — none of them clicked an actual button, typed into an actual input, or read an actual rendered DOM. The backup import conflict UI built earlier this session (entries #20, #26) had zero UI-level test coverage as a result.

**Fix (test infrastructure, not a production code change):** this project's Vitest config runs in a Node environment with no JSX transform, so `.tsx` component files could not previously be imported by any test at all (confirmed by trying — it fails with an explicit esbuild parse error). Installed `jsdom`, `@testing-library/react`, `@testing-library/user-event`, and `@vitejs/plugin-react` (the last already a transitive dependency of this project's own `@astrojs/react` integration, now added directly) as dev dependencies; added the React plugin to `vitest.config.ts` and extended its `include` glob to also pick up `.test.tsx` files. Each harness test file opts into a `jsdom` environment via a per-file `// @vitest-environment jsdom` pragma, so the rest of the suite stays on the faster, unaffected `node` environment. The harness imports `ProApp` directly — bypassing `ProGate` is not a modification to `ProGate`, a shipped bypass flag, or an exposed test credential; it's the same "test the component in isolation" pattern used throughout React testing generally, and `ProGate`'s own real entitlement-checking code (fixed in entry #23) is completely untouched.

**Regression tests:** `tests/browser/proApp.harness.test.tsx` (7 tests, all driving real `fireEvent` clicks/typed input against real rendered DOM, backed by real `fake-indexeddb` transactions):
- New project → real "+ New project" click → typed title → real "+ Add room" click → typed room dimensions → real "Save draft" click → verified in actual storage → full component unmount/remount (a fresh instance re-reading from storage, not retained React state) → the same data reappears in the DOM.
- Backup import conflict UI: **Cancel** (storage completely untouched), **Use imported** (storage actually updated), **Keep both** (a genuinely new second project committed, original untouched), and the default **Keep local** (left unselected, still correctly preserves the local project) — all via real radio-button clicks and a real Confirm-button click, exactly the actions the task named.
- The two-tab save conflict: seeded a real version-checked write to simulate "another tab" saving first, triggered a real conflict via the actual `saveDraft()` button, then exercised **Reload latest** (the other tab's save is adopted, this tab's own edit is discarded) and, in a second test, **Save my edit as a copy** (both the other tab's save AND this tab's conflicting edit survive as two separate projects) — via real button clicks, reading the outcome back from real storage.

**Verification (automated):** 339/339 tests pass (7 new); `astro check` 0 errors; `astro build` succeeds — confirming the new dev dependencies and Vitest config change have no effect on the production build.
